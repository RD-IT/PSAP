...
namespace PSAP.DAO.INVDAO
{
    public class FrmSpecialWarehouseWarrantDAO
    {
        ...
        public void QuerySpecialWarehouseWarrantHeadHead(DataTable queryDataTable, string beginDateStr, string endDateStr, string reqDepStr, string repertoryNoStr, int warehouseStateInt, string preparedStr, int approverInt, string commonStr, bool nullTable)
        {
            BaseSQL.Query(QuerySpecialWarehouseWarrantHeadHead_SQL(beginDateStr, endDateStr, reqDepStr, repertoryNoStr, warehouseStateInt, preparedStr, approverInt, commonStr, nullTable), queryDataTable);
        }

        /// <summary>
        /// 查询预算外入库单表头的SQL
        /// </summary>
        public string QuerySpecialWarehouseWarrantHeadHead_SQL(string beginDateStr, string endDateStr, string reqDepStr, string repertoryNoStr, int warehouseStateInt, string preparedStr, int approverInt, string commonStr, bool nullTable)
        {
            string sqlStr = " 1=1";
            if (beginDateStr != "")
            {
            ...
            if (approverInt >= 0)
            {
                ...
                {
                    sqlStr = string.Format("select INV_SpecialWarehouseWarrantHead.* from INV_SpecialWarehouseWarrantHead left join PUR_ApprovalType on INV_SpecialWarehouseWarrantHead.ApprovalType = PUR_ApprovalType.TypeNo where {0} and INV_SpecialWarehouseWarrantHead.WarehouseState in (1, 4) and ((PUR_ApprovalType.ApprovalCat = 0 and exists (select * from(select top 1 * from F_OrderNoApprovalList(INV_SpecialWarehouseWarrantHead.SpecialWarehouseWarrant, INV_SpecialWarehouseWarrantHead.ApprovalType) Order by AppSequence) as minlist where Approver = {1})) or(PUR_ApprovalType.ApprovalCat = 1 and exists(select * from F_OrderNoApprovalList(INV_SpecialWarehouseWarrantHead.SpecialWarehouseWarrant, INV_SpecialWarehouseWarrantHead.ApprovalType) where Approver = {1}))) order by AutoId", sqlStr, approverInt);
                    return sqlStr;
                }
            }
            if (nullTable)
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        for (int i = 0; i < orderHeadRows.Length; i++)
                        {
                            ...
                            string logStr = LogHandler.RecordLog_OperateRow(cmd, "预算外入库单", orderHeadRows[i], "SpecialWarehouseWarrant", "取消审批", SystemInfo.user.EmpName, BaseSQL.GetServerDateTime().ToString("yyyy-MM-dd HH:mm:ss"));
                        }
